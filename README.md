...
2. **初始化项目** — 在项目根目录生成 `./spm` 命令入口、`Packages/` 目录结构及下载脚本。是否把 `Packages/Caches/` 写入 `.gitignore` 由业务方自行决定。
...
> `spm` 是项目根目录内的命令入口，所以默认写法是 `./spm update`。如果团队自行把项目根目录加入 `PATH`，也可以直接执行 `spm update`。

## 注意事项

- `Packages/Caches/` 是否提交由业务方自行决定，spm-local 不会自动修改 `.gitignore`
- 新同事 clone 项目后，跑一次脚本即可恢复所有依赖
- 如果某个库是其他库的 SPM 依赖，也需要一并本地化
