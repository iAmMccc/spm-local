...
2. **初始化项目** — 在项目根目录生成 `Packages/` 目录结构及下载脚本。是否把 `Packages/Caches/` 写入 `.gitignore` 由业务方自行决定。

> 也可以不安装 Skill，纯手动使用：直接把 `scripts/fetch-packages.sh` 和 `packages.json.example` 拷到项目里用即可。

安装时会显示版本号（如 `正在安装 spm-local v1.0.0...`）。重复执行安装命令可升级到最新版：**已存在的 `Packages/packages.json` 不会被覆盖，但下载脚本会强制替换为最新版**，无论本地脚本是否存在或是否改过，并提示版本变化。运行 `./Packages/scripts/fetch-packages.sh` 时也会在开头打印当前版本，方便确认。
...
如果某个依赖没有写 `version`，默认执行脚本时，已存在的缓存会跳过，避免每次下载都改变本地包版本。需要主动更新时使用：

```bash
./Packages/scripts/fetch-packages.sh update all
./Packages/scripts/fetch-packages.sh update SnapKit
```

`update` 不会修改 `packages.json`。已写 `version` 的依赖会同步到清单指定 tag；未写 `version` 的依赖会更新到远端默认分支最新。更新时会把 `Packages/Caches/` 下对应库视为可再生成缓存，直接覆盖其中的本地未提交改动。

## 注意事项

- `Packages/Caches/` 是否提交由业务方自行决定，spm-local 不会自动修改 `.gitignore`
